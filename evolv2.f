...
*
      ...
      REAL*8 m1ce,m2ce,mch,tmsnew,dm22,mew,beta1
...
* Prescription from Belczynski at al., 2008, ApJS, 174, 223
* Adopted from COSMIC (amended interpolation formula!)
                  beta1 = 0.125d0
                  if(kstar(k).le.1)then
                     ...
                     elseif(mass(k).le.1.4d0)then
                        ...
                        beta1 = 
     &                  (7.d0-0.5d0)*((mass(k)-1.4d0)/(120.d0-1.4d0))
     &                         + 0.5d0
                     endif
                  elseif(kstar(k).ge.7.and.kstar(k).le.9)then
                     ...
                     elseif(mass(k).le.10.d0)then
                        ...
                        beta1 =
     &                  (7.d0-0.125d0)*((mass(k)-10.d0)/(120.d0-10.d0))
     &                               + 0.125d0
                     endif
                  endif
               endif
               vwind2 = 2.d0*beta1*acc1*mass(k)/rad(k)
...
*
     ...
     &             k3*massc(k)*radc(k)*radc(k))*ospbru
         if(jspin(k).gt.jspbru.and.ABS(dtm).gt.tiny
     &      .and.kstar(k).ne.14)then
            mew = 1.d0
...
*           WRITE(0,*)' MASS EXCEEDED ',mass1i,mass2i,tbi,ecci,mt
*           goto 140
*        endif
*
...
*
         if(rad(j1).ge.1.002d0*rol(j1))then
            ...
            if(intpol.ge.100)then
               WRITE(0,*)' INTPOL EXCEEDED ',mass1i,mass2i,tbi,ecci
               goto 140 
            endif
...
*
      if(iter.ge.loop)then
         WRITE(0,*)' MAXIMUM ITER EXCEEDED ',mass1i,mass2i,tbi,ecci
         goto 140
      endif
      goto 5
*
...
*
* Set a flat qcrit (override qc from above)
*
      if (qcr0.lt.0.0d0) qc = -qcr0
*
...
*
     ...
     &        (kstar(j1).eq.2.and.q(j1).gt.qc.and.hgflag.ge.0).or.
...
*
         if(kstar(j1).ge.2.and.kstar(j1).le.9.and.kstar(j1).ne.7.and.
     &      (kstar(j1).ne.2.or.q(j1).le.qc.or.hgflag.ge.0))then
***
...
***
     ...
     &          kstar(j2).le.1.and.q(j1).gt.qc).or.
     &          (kstar(j1).eq.2.and.q(j1).gt.qc.and.hgflag.lt.0))then
*
...
* Adopted from COSMIC (amended interpolation formula!)
     ...
     &                         mass(k),massc(k),rlperi,z)
               vwind2 = 2.d0*beta1*acc1*mass(k)/radx(k)
               omv2 = (1.d0 + vorb2/vwind2)**(3.d0/2.d0)
               dmt(3-k) = ivsqm*acc2*dmr(k)*((acc1*mass(3-k)/vwind2)**2)
     &                    /(2.d0*sep*sep*omv2)
...
*
* Introduce a mass accretion efficiency FA
* (or 'f_a' in Belczynski et al., 2008, ApJS, 174, 223)  
* for non-compact or non-degenerate secondary.
* Exclude the case of nova eruption.
*
* Allow user-input FA to lie between 0.0 and 1.0; otherwise
* simply switch to default FA=1
*
         if(FA.gt.1.0d0.or.FA.lt.0.0d0) FA = 1.0d0
         if(kstar(j2).lt.10)then
            dm2 = FA*dm2
            if(.not.novae) dm22 = FA*dm22
         endif
*
...
*
     ...
     &                k3*massc(k)*radc(k)*radc(k))*ospbru
            if(jspin(k).gt.jspbru.and.kstar(k).ne.14)then
               mew = 1.d0
...
*
      ...
      bpp(jp,12) = ospin(2)
*
      if((kstar(j1).eq.2.and.hgflag.ge.0).or.
     &   (kstar(j1).ge.3.and.kstar(j1).le.9.and.kstar(j1).ne.7))then
         CALL comenv(mass0(j1),mass(j1),massc(j1),aj(j1),jspin(j1),
     &               kstar(j1),mass0(j2),mass(j2),massc(j2),aj(j2),
     &               jspin(j2),kstar(j2),zpars,ecc,sep,jorb,coel)
         com = .true.
      elseif((kstar(j2).eq.2.and.hgflag.ge.0).or.
     &       (kstar(j2).ge.3.and.kstar(j2).le.9.and.kstar(j2).ne.7))then
...
*
      ...
      tb = tb*yeardy
      if(jp.ge.80)then
         WRITE(0,*)' EVOLV2 ARRAY ERROR ',mass1i,mass2i,tbi,ecci
         WRITE(*,*)' STOP: EVOLV2 ARRAY ERROR '
         CALL exit(0)
         STOP
      elseif(jp.ge.40)then
         WRITE(0,*)' EVOLV2 ARRAY WARNING ',mass1i,mass2i,tbi,ecci,jp
      endif
      bcm(ip+1,1) = -1.0
      bpp(jp+1,1) = -1.0
*
