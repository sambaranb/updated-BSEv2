...
*** Input parameters from stdin *** 
      READ(5,*)mass0(1),mass0(2),tphysf,tb,kstar(1),kstar(2),z,ecc
      READ(5,*)neta,bwind,hewind,wconst,alpha1,lambda
      READ(5,*)ceflag,tflag,ifflag,wdflag,bhflag,nsflag,mxns,idum
      READ(5,*)psflag,kmech,ecflag,edflag,wrflag
      READ(5,*)pts1,pts2,pts3
      READ(5,*)sigma,beta,xi,acc2,epsnov,ftzacc,fmrg,eddfac,gamma
      READ(5,*) dtp, qcr0, FA, hgflag
      if(kstar(1).lt.0.or.kstar(2).lt.0)then
         READ(5,*)tphys
         READ(5,*)aj,mass(1),ospin(1)
         epoch(1) = tphys - aj
         kstar(1) = ABS(kstar(1))
         READ(5,*)aj,mass(2),ospin(2)
         epoch(2) = tphys - aj
         kstar(2) = ABS(kstar(2))
      else
*
...
*
 ...
 30   j = j + 1
      ...
      kw = INT(bcm(j,2))
      kw2 = INT(bcm(j,16))
      WRITE(6,99)bcm(j,1),kw,kw2,bcm(j,4),bcm(j,18),
     &            bcm(j,8),bcm(j,22), 
     &            bcm(j,6),bcm(j,20),bcm(j,15),bcm(j,29),
     &            bcm(j,5),bcm(j,19),bcm(j,7),bcm(j,21),
     &            bcm(j,13),bcm(j,27),
     &            bcm(j,14),bcm(j,28),
     &            bcm(j,30),bcm(j,31),
     &            bcm(j,32)
      if(bcm(j,1).ge.0.0) goto 30
 99   FORMAT(f10.4,2i3,12f10.4,6e12.4,f12.5)
...
*
 50   j = 0
      WRITE(6,*) '###'
      WRITE(6,*)'     TIME      M1       M2   K1 K2        SEP    ECC',
     &          '  R1/ROL1 R2/ROL2    SPIN1/KERR1 SPIN2/KERR2   TYPE'
 52   j = j + 1
      if(bpp(j,1).lt.0.0) goto 60
      kstar(1) = INT(bpp(j,4))
      kstar(2) = INT(bpp(j,5))
      kw = INT(bpp(j,10))
      bpp(j,8) = MIN(999.999d0,bpp(j,8))
      bpp(j,9) = MIN(999.999d0,bpp(j,9))
      WRITE(6,100)(bpp(j,k),k=1,3),kstar,(bpp(j,k),k=6,9),
     &  bpp(j,11),bpp(j,12),label(kw)
      goto 52
 60   continue
 100  FORMAT(f11.4,2f9.3,2i3,f13.3,f6.2,2f8.3,2x,1p,2e12.4,3x,a8)
      WRITE(6,*) '###'
*
************************************************************************
*
